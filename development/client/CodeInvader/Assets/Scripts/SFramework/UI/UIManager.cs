...
Description:
    ...
    使用：我们提供了一个默认的Canvas，放在Resources文件夹下，Canvas下的UI会分别保存在Normal, Fixed, Popup层级下。
           ...
           3.写好button等的事件，调用UIManager的方法ShowUI和CloseUI来开关UI
...
namespace SFramework
{
    ...
    public class UIManager : IGameMgr {
        ...
        public UIManager(GameMgr gameMgr) : base(gameMgr)
        {
            //字段初始化
        ...
        public override void Awake()
        {
            ...
            {
                // Global UI
                formToPathDict.Add("Debug", @"UI\Debug");
                formToPathDict.Add("Loading", @"UI\LoadingInterface");

                // Login Scene UI
                formToPathDict.Add("Welcome", @"UI\WelcomeInterface");
                formToPathDict.Add("FastLogin", @"UI\FastLogin");
                formToPathDict.Add("Login", @"UI\LoginInterface");
                // Main Scene UI
                ////// Before Battle
                formToPathDict.Add("Main", @"UI\MainInterface");
                formToPathDict.Add("RoomSelection", @"UI\RoomSelection\RoomSelection");
                formToPathDict.Add("RoomPreparation", @"UI\RoomPreparation\RoomPreparation");
                ////// After Battle
                formToPathDict.Add("BattleResult", @"UI\BattleResult");

                // Battle Scene UI
                formToPathDict.Add("BattleInfo", @"UI\BattleInfo");
                formToPathDict.Add("BattleOperation", @"UI\BattleOperation");
                formToPathDict.Add("CodingOperation", @"UI\CodingOperation");
                formToPathDict.Add("ResourceControl", @"UI\ResourceControl");
                formToPathDict.Add("Backpack", @"UI\BackPackResource\BackPack");
                formToPathDict.Add("Assembly", @"UI\Assembly");
                formToPathDict.Add("FullMap", @"UI\MapInfo");

                formToPathDict.Add("Joystick", @"UI\FloatingJoystick");
                formToPathDict.Add("TouchInput", @"UI\TouchInput");
        ...
        public ViewBase ShowUI(string uiFormName)
        {
	    ...
	    {
	        CanvasGO = gameMgr.resourcesMgr.LoadAsset(Common.SYS_PATH_CANVAS, false);
	    }
	    ...
	    private ViewBase LoadFormsToAllUIFormsCatch(string uiFormsName)
	    {
            ...
            if (baseUIResult==null)
	        {
                ...
                if (baseUIResult == null)
                    return baseUIResult;
                // 赋值引用
                baseUIResult.UiManager = this;
                baseUIResult.UiMaskMgr = UiMaskMgr;
	        }
	    ...
	    private ViewBase LoadUIForm(string uiFormName)
        {
            ...
            {
                goCloneUIPrefabs = gameMgr.resourcesMgr.LoadAsset(strUIFormPaths, false);
            }
            ...
            Debug.Log("创建失败，该UI可能未添加到UiManager中："+uiFormName);
            return null;
        }//Mehtod_end
